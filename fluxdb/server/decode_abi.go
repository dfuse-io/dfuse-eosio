...
func (srv *EOSServer) decodeABIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if tableDef == nil {
		writeError(ctx, w, fluxdb.DataTableNotFoundError(ctx, account, tableName))
		return
	}
	...
	for _, hexRow := range request.HexRows {
		...
		if err != nil {
			writeError(ctx, w, fluxdb.DataDecodingRowError(ctx, hexRow))
			return
		}
	...
	zlog.Debug("streaming response", zap.Int("row_count", len(response.DecodedRows)), zap.Uint32("block_nun", response.BlockNum))
