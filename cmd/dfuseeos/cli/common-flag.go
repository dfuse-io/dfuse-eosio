...
func init() {
	launcher.RegisterCommonFlags = func(cmd *cobra.Command) error {
		...
		// Filtering
		cmd.Flags().StringSlice("common-include-filter-expr", []string{"*"}, "[COMMON] CEL program to determine if a given action should be included for processing purposes, can be used multiple times with low blocknum prefix `#123;`, see https://docs.dfuse.io/eosio/admin-guide/filtering/ for more information.")
		cmd.Flags().StringSlice("common-exclude-filter-expr", []string{""}, "[COMMON] CEL program to determine if an included action should be excluded, can be used multiple times with low blocknum prefix `#123;`, see https://docs.dfuse.io/eosio/admin-guide/filtering/ for more information.")
		cmd.Flags().StringSlice("common-system-actions-include-filter-expr", []string{"receiver == 'eosio' && action in ['updateauth', 'deleteauth', 'linkauth', 'unlinkauth', 'newaccount', 'setabi']"}, "[COMMON] CEL program to determine which actions to keep regardless of the include or exclude filter expressions, those are actions required by dfuse system(s) to function properly, can be used multiple times with low blocknum prefix `#123;`, change it only if you known what you are doing, see https://docs.dfuse.io/eosio/admin-guide/filtering/ for more information.")
