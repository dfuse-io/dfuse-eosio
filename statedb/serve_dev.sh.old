...
function usage {
    echo "./serve_dev.sh [fluxdbExtraArgs...]"
    exit 1
}
...
#     fi
    ...
    #storeDsnArg="--store-dsn=bigtable://dev.dev/test-v0?createTables=false"
    #storeDsnArg="--store-dsn=bbolt://fluxdb.bbolt"
    storeDsnArg="--store-dsn=badger:///Users/julien/codebase/dfuse/fluxdb/badger.db"

    cmd="./fluxdb serve $storeDsnArg --dev-no-pipeline=true $@"

    echo "About to serve using '$cmd'"
    $cmd
}
