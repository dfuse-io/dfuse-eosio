...
func dfuseStartE(cmd *cobra.Command, args []string) (err error) {
	...
	case <-launch.Terminating():
		...
		if isClean, appName := launcher.IsCleanShutdownError(launch.FirstAppError); isClean {
			userLog.Printf("App %s triggered clean shutdown: %s, shutting down process without error", appName, launch.FirstAppError)
		} else {
