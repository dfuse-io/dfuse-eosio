...
func GetBlocksHandler(db eosrest.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()

		errors := validateBlocksRequest(r)
		if len(errors) > 0 {
			eosrest.WriteError(w, r, derr.RequestValidationError(r.Context(), errors))
			//////////////////////////////////////////////////////////////////////
		...
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to get blocks"))
			return
		}
		...
		for _, block := range dbBlocks {
			...
			if err != nil {
				eosrest.WriteError(w, r, err)
				return
			}
			blockSummaries = append(blockSummaries, blkSummary)
		}

		eosrest.WriteJSON(w, r, blockSummaries)
...
func GetBlockHandler(db eosrest.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		if err != nil || len(id) != 64 {
			eosrest.WriteError(w, r, derr.RequestValidationError(r.Context(), url.Values{
		...
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to get block"))
			return
		}

		blockSummary, err := mdl.ToV1BlockSummary(dbBlock)
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to to convert block"))
			return
		}

		dbSiblingBlocks, err := db.ListSiblingBlocks(r.Context(), blockSummary.BlockNum, 5)
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to get sibling blocks"))
			return
		}
		...
		for i, dbSiblingBlock := range dbSiblingBlocks {
			...
			if err != nil {
				eosrest.WriteError(w, r, derr.Wrap(err, "failed to convert block"))
				return
			}
			blockSummary.SiblingBlocks[i] = v1Block
		}

		eosrest.WriteJSON(w, r, blockSummary)
...
func GetBlockTransactionsHandler(db eosrest.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to get block transactions"))
			return
		}

		eosrest.WriteJSON(w, r, dbTransactionList)
