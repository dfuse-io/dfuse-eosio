...
func GetTransactionHandler(db eosrest.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed to get transaction"))
			return
		}

		transactionLifecycle, err := mdl.ToV1TransactionLifecycle(transaction)
		if err != nil {
			eosrest.WriteError(w, r, derr.Wrap(err, "failed transform transaction to model"))
			return
		}

		eosrest.WriteJSON(w, r, transactionLifecycle)
