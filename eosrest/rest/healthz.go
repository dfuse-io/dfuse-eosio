...
type Healthz struct {
	...
	Healthy struct {
		...
		StateDB *bool `json:"statedb,omitempty"`
		Search  *bool `json:"search,omitempty"`
	} `json:"healthy"`
}

func SearchNotStuckHandler(searchEngine *eosrest.SearchEngine) http.Handler {
...
func HealthzHandler(hub *hub.SubscriptionHub, api *eos.API, blocksStore dstore.Store, db eosrest.DB, stateClient pbstatedb.StateClient, searchEngine *eosrest.SearchEngine, expectedSecret string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func healthCheckSearch(ctx context.Context, h *Healthz, wg *sync.WaitGroup, searchEngine *eosrest.SearchEngine, headBlockNum uint32, zlogger *zap.Logger) {
	defer wg.Done()
...
func healthCheckBigTable(h *Healthz, wg *sync.WaitGroup, db eosrest.DB, r *http.Request, headBlockNum uint32) {
	defer wg.Done()
	done := make(chan struct{})
...
func healthCheckStateDB(ctx context.Context, h *Healthz, wg *sync.WaitGroup, stateClient pbstatedb.StateClient, headBlockNum uint32) {
	...
	go func() {
		...
		h.Healthy.StateDB = newBool(isHealthy)
	...
	case <-time.After(maxSubsystemResponseTime):
		...
		h.Healthy.StateDB = newBool(false)
	}
