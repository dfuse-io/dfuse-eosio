...
func SimpleSearchHandler(db eosrest.DB, blockmetaClient *pbblockmeta.Client) http.Handler {
	hexRegex := regexp.MustCompile(`^[0-9a-fA-F]+$`)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		query := r.FormValue("q")
		if query == "" {
			eosrest.WriteError(w, r, derr.RequestValidationError(ctx, url.Values{"q": []string{"query parameter should not be empty"}}))
			//////////////////////////////////////////////////////////////////////
		...
		eosrest.WriteError(w, r, derr.HTTPNotFoundError(ctx, nil, derr.C("simple_search_not_found"), "no results found for query"))
